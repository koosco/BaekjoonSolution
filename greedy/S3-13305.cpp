/*
	...
	[풀이]
	* 알고리즘
		1. N, 거리, 리터당 가격을 차례대로 입력받는다
			: 마지막 주유소는 리터당 가격을 신경쓰지 않아도 된다
			: 처음에는 무조건 첫 번째 주유소에서 주유해야 한다
		2. 거리와 리터당 가격 배열을 함께 순회한다
		3. 결과에 다음 주유소까지의 거리 x 현재 가격을 더한다
		4. 다음 주유소에 도착하면 현재 가격과 다음 주유소의 가격을 비교한다
			1) 다음 주유소의 가격이 더 저렴하다면 현재 가격을 변경한다
	
	[이력]
	1. 부분통과
		: unsigned int로 data type을 설정했더니 N이 커질 때 오버플로우 발생
	2. o
		: unsigned long으로 data type 변경
*/
...
int main() {
	...
	unsigned long N, currentOilPrice, res = 0;

	cin >> N;
	vector<int> distance(N), price(N);
	...
	currentOilPrice = price[0];
	
	for(int i=1;i<N;i++) {
		res += currentOilPrice * distance[i-1];
		if(price[i] < currentOilPrice)
			currentOilPrice = price[i];
	}
